...
<h5 class="title categoryBar">전체학기 성적 조회</h5>
...
<div class="card">
	...
	<div class="container">
		...
		<table class="table table-bordered">
			...
			<tbody>
			<%-- <div>${fn:length(myClsemesterList)}</div> --%>
				<!-- model.addAttribute("myCourseListtt", myCourseListtt);
					 model.addAttribute("myClsemesterList", myClsemesterList); 
				 -->
			<%-- 	<c:forEach var="semester" items="${myClsemesterList}" varStatus="stat">
					<tr>
						<td rowspan="${fn:length(myClsemesterList)}">${semester.clSemester}</td>
					...
					<c:forEach var="myCourse" items="${myCourseListtt}" varStatus="stat">
						...
						<c:if test="${myCourse.sbjRs eq null}">
							<tr>	
								...
								<!-- 
								최대수강가능학점 : 이전 학기의 myCourse.avgclFscoree에 의해 결정됨
								 -->   
									취득학점/신청학점 ${myCourse.sumsbjRecode}/ ${myCourse.rslt} &nbsp; 평균학점 ${myCourse.avgclFscoree}/4.5
								</th>
							</tr>
						</c:if>
					</c:forEach>
