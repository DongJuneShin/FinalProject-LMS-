...
<script>
	...
	$(function() {
		...
		function handleImgFileSelect(e){
			...
			fileArr.forEach(function(f){
				...
				reader.onload = function(e){
					...
					//alert($(img_html).height());
				}
				//이미지를 읽자
				reader.readAsDataURL(f);
				//alert($(".imgs_wrap").innerHeight())
			});//end forEach
		}
		...
		$("#exExam").on("click",function(){
			...
			var str = "";
			for(var i =1; i<=10; i++){
				str += "<div id='ex_"+i+"' style='margin-left:auto; margin-right:auto; border-radius: 8px; background:white; width:68%; height : 350px;'>";
				str += "<br id='brbr"+i+"'>";
				str += "<input placeholder='질문' value='질문"+i+"' id='que_"+i+"' style='width:80%; height:40px; border:none; margin-left : 20px; background: #e2e1e3; border-bottom: 1px solid #9b9a9c;' type='text'>";
				str += "<br>";
				str += "<br>";
				/* if(i == 1){
					str += "<img alt='' src='/resources/images/1번_화학입자문제.png'>";
				} */
				for(var j=1; j<=4; j++){
					str += "<div id='queDiv_"+i+"-"+j+"'>";
					if(j==1){
						str +=   "<input name='que"+i+"'  id='queCheck_"+i+"-"+j+"' type='radio' class='que"+i+"' style='margin-left : 20px;' checked/>";
					}else{
						str +=   "<input name='que"+i+"'  id='queCheck_"+i+"-"+j+"' type='radio' class='que"+i+"' style='margin-left : 20px;'/>";
					}
					str +=   "<input type='text' id='queCheck_"+i+"-"+j+"value' value='질문"+i+"의"+j+"' class='queText' placeholder='옵션' style='width:350px; height:40px; border:none; margin-left : 20px;' />";
					str += "</div>";
				}
				str +=   "<div id='queAnswerDiv"+i+"'>";
				str +=   	"<span style='margin-left : 20px'>정답 : </span>";
//				str +=   	"<input type='text'value='"+$("input[name=que"+i+"]").prop("checked",true)+"' id='queAnswer"+i+"' readonly/>";
				str +=   	"<input type='text'value='질문"+i+"의1' id='queAnswer"+i+"' readonly/>";
				str += 	"</div>";

				str += 		"<div id='buttonDiv_"+i+"' style='margin-left : 20px;'>";
				str += 			"<button type='button' id='optionAdd_"+i+"' class='btn btn-light'>옵션추가</button>";
				str += 			"<button type='button' id='optionMinus_"+i+"' class='btn btn-light'>옵션삭제</button>";
				str += 			"<button type='button' id='answer_"+i+"' class='btn btn-light'>정답</button>";
				str += 		"</div>";
				str += "</div>";
				str += "<br>";
				str += "<br>";
