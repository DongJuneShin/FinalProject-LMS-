			...
			<tbody>
				<c:forEach var="study" items="${wholeStudyList}" varStatus="stat">
					<tr>
						...
						<td>
							<button type="button" id="${study.S_NUM}" class="btn btn-primary btn-sm btnApplication">신청</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
							...
							<tbody>
								<c:forEach var="student" items="${studentList}">
									<tr>
										...
										<td>
											<button type="button" id="${student.STU_NUM}" class="btn btn-primary btn-sm btnAdd">추가</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
...
$(document).on("click", ".btnAdd", function() {
	...
	openInvite.DataTable().row.add([stuNum, stuNmKor, '<button type="button" id="' + stuNum + '" class="btn btn-primary btn-sm btnCancel">취소</button>']).draw();
	modalTableSetting();
});
...
$(document).on("click", ".btnCancel", function() {
	...
	openList.DataTable().row.add([stuNum, stuNmKor, '<button type="button" id="' + stuNum + '" class="btn btn-primary btn-sm btnAdd">추가</button>']).draw();
	modalTableSetting();
});
...
function checkOpenForm() {
	...
	if(str == 'No data available in table,') str = "";
...
function modalTableSetting() {
	...
	openList_thead.find("tr").children().eq(2).css("width", "70px");
	...
	for(var i = 0; i < openList_tbody.length; i++){
		...
		openList_tbody.eq(i).children().eq(2).css("width", "70px");
	}
	
	let openInvite_thead = openInvite.find("thead");
	openInvite_thead.find("tr").children().css("width", "315px");
	openInvite_thead.find("tr").children().eq(2).css("width", "70px");
	...
	for(var i = 0; i < openInvite_tbody.length; i++){
		...
		openInvite_tbody.eq(i).children().eq(2).css("width", "70px");
	}
