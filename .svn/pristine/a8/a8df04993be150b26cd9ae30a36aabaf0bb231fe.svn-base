...
function updateData(){
	...
	$.ajax({
		...
		success: function(data){ 
			...
			if(alarmCnt < data){
				alarmCnt = data;
				console.log("바뀐 alarmCnt : "+ alarmCnt);
				
// 				Swal.fire({
// 				  icon: 'info',
// 				  html: '<a href="/msg/messageList">안 읽은 메시지가 있습니다.</a>',
// 				  toast: true,
// 				  position: 'top-end',
...
// 				divData += '<i class="email-list-item-bookmark bx bx-star d-sm-inline-block d-none cursor-pointer mx-4 bx-sm"></i>';
				...
				divData += '<span class="email-list-item-username me-2 h6 det">'+ item.sendNm +'</span>';
...
// 				divData += '<i class="email-list-item-bookmark bx bx-star d-sm-inline-block d-none cursor-pointer mx-4 bx-sm"></i>';
				...
				divData += '<img src="/resources/upload'+ item.photo +'" alt="user-avatar" class="d-block flex-shrink-0 rounded-circle me-sm-3 me-0" height="32" width="32">'; 
				divData += '<div class="email-list-item-content ms-2 ms-sm-0 me-2 det">';
				divData += '<span class="email-list-item-username me-2 h6">'+ item.revNm +'</span>';
				divData += '<span class="email-list-item-subject d-xl-inline-block d-block">'+ item.noteTitle +'</span>';
				divData += '</div>';
				divData += '<div class="email-list-item-meta ms-auto d-flex align-items-center det">';
...
<!--             </li> -->
            <li class="d-flex justify-content-between sendLi active" data-target="reception">
              <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                <i class='bx bx-box'></i>
                <span class="align-middle ms-2 recvBox">받은 쪽지함</span>
              </a>
            ...
            <li class="d-flex revLi" data-target="sent">
              <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                ...
                <span class="align-middle ms-2 sendBox">보낸 쪽지함</span>
              </a>
            </li>
            <li class="d-flex selfLi" data-target="my">
              <a href="javascript:void(0);" class="d-flex flex-wrap align-items-center">
                <i class='bx bx-user-pin'></i>
                <span class="align-middle ms-2 myselfBox">내게 쓴 쪽지</span>
              </a>
            </li>
              ...
              <div class="d-flex align-items-center w-100">
                ...
                <div class="mb-0 mb-lg-2 w-100">
                  <div class="input-group input-group-merge shadow-none">
                    ...
                    <input type="text" class="form-control email-search-input border-0 py-0" placeholder="검색어를 입력해주세요" aria-label="Search..." aria-describedby="email-search">
                  </div>
                </div>
              </div>
        ...
        <div class="app-email-view-content py-4">
          ...
          <div class="email-reply card mt-4 mx-sm-4 mx-3 border">
            <h6 class="card-header border-0 ">
            	<button class="btn btn-label-secondary reply">
	            	<i class="bx bx-share me-1"></i>
            ...
            <div class="card-body pt-0 px-3">
	            <form id="replyfrm" name="replyfrm">
	            	<input type="hidden" id="noteSend" name="noteSend"/>
					...
					<textarea class="noteContent1" name="noteContent" id="noteContent1"></textarea>
					<div class="d-flex justify-content-end align-items-center mt-2">
						<button class="btn btn-label-secondary reply" type="button">
							<i class="bx bx-share me-1"></i>
							<span class="align-middle fw-bold">답장하기</span>
						</button>
					</div>
...
$(function(){
	...
	$(document).on("click",".det",function(){
		...
		console.log( noteNum + "번의 쪽지 클릭!");
		$(".app-email-view").addClass("show");
// 		console.log(noteNum);
// 		let data = {'noteNum' : noteNum};
// 		console.log("data : "+JSON.stringify(data));
		var jasonData = {"noteNum": noteNum, "userId" : userId}
		 $.ajax({
				type: "post",
...
// 						$(".revNum").html("To. "+data.revNm + "</b> &nbsp;[ " + data.noteRev + " ]");
...
// 						console.log(formattedDate);
	...
	$(".reply").on("click",function(){
		console.log("답장 클릭했다!");
// 		var queryString = $("form[name=replyfrm]").serialize() ;
		var data = new FormData($("form[name=replyfrm]"));
		console.log(data);
