...
// 			console.log("바뀌기 전 alarmCnt : "+ alarmCnt);
			if(alarmCnt==-1){
				alarmCnt = data;
				localStorage.setItem("alarmCnt", alarmCnt);
// 				console.log("1.바뀐 alarmCnt : "+ alarmCnt);
				Swal.fire({
				  icon: 'info',
				...
				alarmCnt = data;
				localStorage.setItem("alarmCnt", alarmCnt);
// 				console.log("2.바뀐 alarmCnt : "+ alarmCnt);

				Swal.fire({
				  icon: 'info',
				  html: '<a href="/msg/messageList">새로운 메시지가 도착했습니다.</a>',
				  toast: true,
				  position: 'top-end',
				  timerProgressBar: true,
				  showConfirmButton: false,
...
// 		contentType:"application/json;charset=utf-8",
		...
		success: function(data){
			...
			$.each(data, function(i, item){
				...
				unreadData += '<p class="mb-0">' + item.noteSend + '</p>                                                                   ';
				unreadData += '<small class="text-muted">' + item.noteDt + '</small>                                                       ';
				unreadData += '</div>                                                                                                      ';
		...
		<ul class="navbar-nav flex-row align-items-center ms-auto">
			...
			<li class="nav-item navbar-dropdown dropdown-user dropdown">
				...
				<ul class="dropdown-menu dropdown-menu-end">
					...
					<li>
						<a class="dropdown-item" onclick="clickMenuItem()">
...
function clickMenuItem(){
	var listItemA = document.querySelectorAll('.bx-text');
	console.log("클릭 헤더>마이페이지 : " + listItemA.innerHtml);
	listItemA.click();
}
</script>
