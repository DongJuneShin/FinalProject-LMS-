				...
				<tbody>
					<c:forEach var="subjectVO" items="${subjectVOList}" varStatus="status">
						<tr>
							...
							<td>
								<button type="button" class="btn btn-label-info btn-sm btnCheck" 
									data-bs-toggle="modal" data-bs-target="#spModal">상세</button>
							</td>
							<td>
								<c:if test="${subjectVO.sbjYn < 1}">
									<button type="button" class="btn btn-info btn-sm btnOK" onclick="btnOK('${subjectVO.sbjNum}', '${subjectVO.sbjTitle}')">승인</button>
									<button type="button" class="btn btn-dark btn-sm btnCom" 
...
function btnOK(sbjNum, sbjTitle) {
	let check = confirm("\"" + sbjTitle + "\" 강의를 승인하시겠습니까?");
