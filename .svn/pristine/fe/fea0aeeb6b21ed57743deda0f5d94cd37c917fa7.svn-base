			...
			success : function(result){
				...
				queList = result.examAllList[0].examSelList.length+1;
			 	...
			 	for(var i =0; i<result.examAllList.length; i++){
				    ...
				    str+= 		"<br>";
				    str+= 		"&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-size : 25px;'>"+parseInt(i+1)+".</span><input placeholder='질문' readonly id='que_"+parseInt(i+1)+"' value='"+result.examAllList[i].examqueQue+"' style='width:80%; height:40px; border:none; margin-left : 20px; border-bottom: 1px solid #9b9a9c;' type='text' />";
				    str+= 		"<br>";
				    str+= 		"<br>";
	  ...
	  $("#confirm").on("click",function(){
			  ...
			  $.ajax({
					...
					success : function(result){
						...
						//alert("맞은갯수 : "+trueCount + " , 틀린갯수 : "+falseCount);
					}
				});//ajax끝
	  })//confirm 끝
})
